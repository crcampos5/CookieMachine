...
class BobEsponjaFace:
    ...
    def execute(self,imagen : Imagen):
        ...
        imageHSV = cv.cvtColor(self.imagen.imagen, cv.COLOR_BGR2HSV)
        ...
        imagen = cv.inRange(imageHSV, color_bajos, color_altos)
        #se extrae los contornos de la imagen 
        cnts,_ = cv.findContours(imagen, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
        #img = self.imagen.get()
        for c in cnts:
            area = cv.contourArea(c)
            if area < 3700 or 3000000 < area:
                continue
