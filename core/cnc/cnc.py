...
class Cnc(Subject):
    ...
    def connect_serial(self, port, is_open):
        ...
        if self.isconect is True:
            self.conection = serial.Serial(
                self.port, baudrate=115200, timeout=2)
    ...
    # envía la señal de "home" a la máquina CNC para mover sus ejes a las
    ...
    # en el eje X e Y, capturar imágenes con una cámara y devolver una lista
    # de imágenes escaneadas.

    def move_scan(self, x, y, cap, d=100):
        # Obtiene la compensacion para las coordenadas X e Y del modo de escaneo láser.
        offset_xy = self.modes["Laser"]
        # Calcula la coordenada X inicial para el escaneo
        c = x - d + offset_xy[0] + 1
        c_final = c + d
        print("c inicial: ", c)
        # Ajusta las coordenadas x e y con el offset obtenido anteriormente.
        x = str(x + offset_xy[0])
        y = str(y + offset_xy[1])
        ...
        # Comprueba si la posición actual coincide con la posición deseada
        # para escanear.
        # Si coincide, captura una imagen utilizando la cámara y
        # la agrega a list_images.
        num_images = 0
        while 1:
            ...
            if "MPos" in data:
                ...
                if c == c_final:
                    break
                if c < w:
                    raise ValueError("Hubo una descoordinacion al escanear")

        end_time = time.time()

        print("Time: ", end_time - start_time)
        # num_imagenes_almacenadas = np.sum(list_images != None)
        num_imagenes_almacenadas = len(list_images)
        print("Cantidad de imagenes: ", num_imagenes_almacenadas)
        self.laser_onoff(False)
        return list_images
    ...
    def laser_onoff(self, ban: bool):
        if self._check():
            ...
            if self.laser_on:
                ...
                print(data)
                data = self._send("M3")
                #thread = threading.Thread(target=self.state)
                #thread.start()
                #thread.join()
            else:
                data = self._send("M5")
    ...
    def sweep_area(self,cap):
        if self.ishome :
            ...
            rango_x = range(int(self.pos.X), limits["x"], step_x)
            rango_y = range(int(self.pos.Y), limits["y"], step_y)
            ...
            pattern = r"MPos:([-+]?\d+\.\d+),([-+]?\d+\.\d+),([-+]?\d+\.\d+)"

            for c in coordenadas:  
                self.conection.write((f"G0 X{c[0]} Y{c[1]} \n").encode())
                self.conection.reset_input_buffer()
                ...
                while 1:
                    ...
                    if matches:
                        x = round(float(matches.group(1)),1)
                        y = round(float(matches.group(2)),1)
                        #z = float(matches.group(3))
                        if int(x) == int(c[0]) and int(y) == int(c[1]):
                            print(f"captura: {x} {y}")
                            ret, imagen = cap.read()
                            if ret:
                                list_images.append(imagen)
                                ...
                                #num_images += 1
                            break
            return list_images, coordenadas
